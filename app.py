...
# ================= 🔍 CUSTOM SINGLE COIN ANALYZER =================
st.subheader("🔍 Custom Coin Analyzer (On-Demand AI Inspection)")
custom_col1, custom_col2 = st.columns([3, 1])

with custom_col1:
    custom_symbol_input = st.text_input("Coin නම ඇතුළත් කරන්න (උදා: SOL, DOGE, PEPE, ETH, XRP):", value="SOL").strip().upper()

with custom_col2:
    st.write("##")
    analyze_custom_btn = st.button("Analyze Coin 🚀", use_container_width=True)

if analyze_custom_btn and custom_symbol_input:
    target_pair = f"{custom_symbol_input}USDT"
    with st.spinner(f"{target_pair} දත්ත සහ AI නිර්දේශය ගණනය කරමින් පවතී..."):
        try:
            ...
            if ticker_res.status_code == 200 and kline_res.status_code == 200:
                ...
                c_ema = df_custom['close'].ewm(span=20, adjust=False).mean().iloc[-1]
                c_atr = calculate_atr(df_custom, period=14)
                c_pattern = detect_candlestick_pattern(df_custom)
                ...
                signal_type_guess = "PUMP" if real_time_price >= c_ema else "DUMP"
                ...
                if enable_ai and gemini_key:
                    ai_verdict, ai_reason, ai_risk = analyze_with_ai(
                        ...
                        c_vol_spike, f"{c_rsi:.1f}", c_pattern, c_dom_str, btc_msg, gemini_key
                    )
                
                # Metrics Display
                st.markdown(f"### 📌 Analysis for **{custom_symbol_input}/USDT**")
                m1, m2, m3, m4, m5 = st.columns(5)
                m1.metric("Live Price ($)", f"${real_time_price:,.4f}")
                m1_delta = f"{c_change:+.2f}%"
                m2.metric("15m Change", m1_delta, delta=m1_delta)
                m3.metric("RSI (14)", f"{c_rsi:.1f}")
                m4.metric("Volume Spike", c_vol_spike)
                m5.metric("Whale Dominance", f"🟢 {c_buyer_pct}% / 🔴 {c_seller_pct}%")
                ...
                with c_col_ai:
                    ...
                    st.info(f"**Verdict:** {ai_verdict}")
                    st.write(f"**Reason:** {ai_reason}")
                    st.write(f"**Risk Level:** `{ai_risk}`")
                    st.write(f"**Pattern:** `{c_pattern}`")
                    st.write(f"**20 EMA Status:** {'🟢 Above EMA' if real_time_price >= c_ema else '🔴 Below EMA'}")
                    st.write(f"**Long Target 1:** `${tp1_long:,.4f}` | **SL:** `${sl_long:,.4f}`")
                    st.write(f"**Short Target 1:** `${tp1_short:,.4f}` | **SL:** `${sl_short:,.4f}`")
            else:
                st.error(f"Binance හි `{target_pair}` නමින් Pair එකක් සොයාගත නොහැකි විය. කරුණාකර නිවැරදි Symbol එක යොදන්න.")
...
st.caption(f"🛡️ Market Context: {btc_msg} | Mode: {scan_mode} | 24/7 Cloud Active")
...
if results:
    ...
    st.markdown("### 📊 Market Live Charts")
