...
def send_telegram_alert(coin, plan, ai_review):
    ...
    msg_html = (
        f"⚡ <b>RSI & AI VERIFIED SCALP SIGNAL</b>\n\n"
        f"🪙 <b>Coin:</b> <code>{coin}</code>\n"
        f"🤖 <b>ChatGPT Audit:</b> <code>{ai_review}</code>\n"
        f"🎯 <b>Direction:</b> {icon} <b>{plan.get('direction')}</b>\n"
        f"📈 <b>RSI (14):</b> <code>{plan.get('rsi_val')} / 100</code>\n"
        f"📊 <b>Order Book:</b> <code>{plan.get('orderbook')}</code>\n\n"
...
def get_orderbook_ratio(raw_symbol):
    try:
        res = requests.get(f"{SPOT_BASE_URL}/depth", params={'symbol': raw_symbol, 'limit': 20}, timeout=3)
        if res.status_code == 200:
            data = res.json()
...
# ----------------- TAB 2: INSTANT SCALP (RSI + ORDERBOOK + AI AUDIT) -----------------
with tabs[1]:
    st.subheader("⚡ Live Instant Scalp Signal Generator (RSI & AI Verified)")
    st.caption("സජීවී RSI අගයන්, Order Book තත්ත්වයන් සහ ChatGPT විග්‍රහය පදනම් කරගත් ප්‍රෝ-ග්‍රේඩ් ස්කැල්ප් සිග්නල් ජනකකය.")

    if st.button("🚀 Fetch RSI Scalp, Audit with AI & Send", use_container_width=True):
        with st.spinner("සජීවී RSI සහ වෙළඳපොළ දත්ත ගණනය කරමින් පවතී..."):
            try:
                res_spot = requests.get(f"{SPOT_BASE_URL}/ticker/24hr", timeout=6)
                if res_spot.status_code == 200:
                    top_vol = sorted([t for t in res_spot.json() if t.get('symbol', '').endswith('USDT')], key=lambda x: float(x.get('quoteVolume', 0)), reverse=True)[:10]
                    found_valid = False
                    for item in top_vol:
                        sym = item['symbol']
                        disp = f"{sym[:-4]}/USDT"
                        ...
                        if k_res.status_code == 200:
                            ...
                            b_pct, s_pct = get_orderbook_ratio(sym)
                            orderbook_str = f"Buyers {b_pct}% / Sellers {s_pct}%"
                            ...
                            # AI Full Verification
                            ai_review = ai_verify_trade_setup(disp, direction, cur_p, rsi_val, orderbook_str)
                            
                            if "VALID" in ai_review.upper():
                                send_telegram_alert(disp, plan, ai_review)
                                st.success(f"✅ RSI & AI Verified! **{disp}** | RSI: {rsi_val} | Orderbook: {orderbook_str}")
                                found_valid = True
                                break
                            else:
                                st.warning(f"❌ AI Rejected **{disp}** (RSI: {rsi_val}): {ai_review}")
                    if not found_valid:
                        st.warning("මේ මොහොතේ නිවැරදි RSI කොන්දේසි සහිත අවස්ථා හමු නොවීය.")
            except Exception as e:
